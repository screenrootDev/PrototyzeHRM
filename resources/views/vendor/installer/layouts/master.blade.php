    ...
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 animate-fade-in">
        <div class="max-w-4xl w-full">
            ...
            <div class="text-center mb-10">
                ...
                <p class="text-lg text-gray-600">{{ trans('installer_messages.title') }}</p>
            </div>
                        ...
                        <div class="flex items-center">
                            <div class="flex flex-col items-center">
                                <div class="w-10 h-10 rounded-full border-2 transition-all duration-300 flex items-center justify-center shadow-sm
                                    {{ $isActive ? 'border-primary-600 bg-primary-600 text-white scale-110 shadow-primary-200' : ($isCompleted ? 'border-primary-600 bg-primary-50 text-primary-600' : 'border-gray-200 bg-white text-gray-400') }}">
                                    <i class="{{ $step['icon'] }} text-sm"></i>
                                </div>
                                <span class="mt-2 text-xs font-semibold {{ $isActive ? 'text-primary-700' : ($isCompleted ? 'text-primary-600' : 'text-gray-400') }} hidden sm:block">{{ $step['label'] }}</span>
                            </div>
                            @if($index < count($steps) - 1)
                                <div class="w-12 sm:w-20 h-1 mx-2 rounded-full transition-all duration-300 {{ $isCompleted ? 'bg-primary-600' : 'bg-gray-200' }}"></div>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                @if(session()->has('errors'))
                    <div class="mb-8 p-5 bg-red-50 border border-red-100 rounded-xl animate-fade-in" id="error_alert">
                        <div class="flex items-start">
                            ...
                            <div class="flex-1">
                                <h4 class="text-red-900 font-bold mb-1">{{ trans('installer_messages.forms.errorTitle') }}</h4>
            ...
            <div class="text-center mt-8 text-gray-400 text-sm">
                &copy; {{ date('Y') }} {{ config('app.name') }} Installer
            </div>
        </div>
    </div>
