...
const CARD_COLUMN_WIDTH = 208;

function leafCount(node: Employee): number {
    const children = node.children?.filter(child => child.name?.trim()) ?? [];

    return children.length
        ? children.reduce((count, child) => count + leafCount(child), 0)
        : 1;
}

function treeWidth(node: Employee): number {
    return Math.max(CARD_COLUMN_WIDTH, leafCount(node) * CARD_COLUMN_WIDTH);
}
...
function NodeCard({ node, isRoot = false }: { node: Employee; isRoot?: boolean }) {
    ...
    return (
        <div
            ...
            style={{ width: `${treeWidth(node)}px`, flexShrink: 0 }}
        >
            {/* Card */}
                ...
                <div className="flex flex-col items-center w-full">
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', position: 'relative', width: '100%' }}>
                        {visibleChildren.map((child, index) => (
                            <div key={child.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: `${treeWidth(child)}px`, flexShrink: 0, position: 'relative' }}>
                                {/* Vertical line */}
                                <div className="bg-gray-300 dark:bg-gray-600"
