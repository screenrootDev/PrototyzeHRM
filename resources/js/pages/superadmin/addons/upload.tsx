...
export default function UploadPackages() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('addons.install'), {
            onSuccess: () => {
                setFiles([]);
                setData('files', []);
                toast.success('Add-ons installed successfully.');
            },
            onError: () => {
                toast.error('Failed to install add-ons.');
            }
        });
    };
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: "Add-ons Manager", url: route('addons.index') },
        { title: "Upload", isActive: true }
    ];

    return (
        <PageTemplate
            title={t('Upload Add-ons')}
            description={t('Upload Add-ons Zip Files')}
            url={route('addons.upload')}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end gap-3">
                            <Button
                                type="button"
                                variant="outline"
                                onClick={() => router.visit(route('addons.index'))}
                            >
                                {t('Cancel')}
                            </Button>
                            <Button
